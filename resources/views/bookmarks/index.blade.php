                ...
                @if ($bookmark->work)
                    @php
                        $continueChapter = $bookmark->last_chapter_read
                            ...
                            : null;
                    @endphp
                    <div class="page-reveal" data-reveal-delay="{{ ($loop->index % 3) + 1 }}" style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 18px; padding: 18px; display: grid; grid-template-columns: 88px 1fr auto; gap: 18px; align-items: center;">
                        <a href="{{ route('works.public.show', $bookmark->work) }}" style="text-decoration:none;">
                            @if ($bookmark->work->cover)
                                <div class="media-shell" style="width: 88px; height: 118px; border-radius: 12px;">
                                    <div class="media-skeleton"></div>
                                    <img src="{{ asset('storage/' . $bookmark->work->cover) }}" alt="{{ $bookmark->work->title }}" style="width: 88px; height: 118px; object-fit: cover; border-radius: 12px; display: block;" data-media-loading loading="lazy">
                                </div>
                            @else
